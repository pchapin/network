...
PROGRAMMER : (C) Copyright 2007 by Peter C. Chapin <PChapin@vtc.vsc.edu>
 ...
 */
void base64_encode( FILE *infile, FILE *outfile )
{
             size_t count;         // Number of bytes read from the input.
    ...
    while( ( count = fread( in_buffer, 1, 3, infile ) ) != 0 ) {
 ...
 * \param outfile The previously opened output file.
 */
void base64_decode( FILE *infile, FILE *outfile )
{
    fprintf( stderr, "base64_decode: Sorry, not implemented!\n" );
}
